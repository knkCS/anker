...
- **`LookupSelect`** — `BaseSelect` for options that live on a server: it composes `BaseSelect` (so the two render identically) and adds debounce, menu-open gating, cancellation, paging and id-to-label resolution around a consumer-supplied `search`. See `Atoms/LookupSelect`.
