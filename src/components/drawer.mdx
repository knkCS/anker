...
<table>
  ...
  <tbody>
    <tr>
      <td>Left-side placement</td>
      <td>Not yet</td>
      <td>Chakra supports <code>placement="start"</code> but it's not exposed as a prop. Pass it via rest props.</td>
    </tr>
    <tr>
      <td>Custom width / size variants</td>
      <td>Not yet</td>
      <td>Uses Chakra's default drawer sizing. Override via <code>size</code> rest prop.</td>
    </tr>
    <tr>
      <td>Cancel button in footer</td>
      <td>Not included</td>
      <td>Only save button shown. Use <code>additionalButtons</code> or <code>footerText</code> for cancel.</td>
    </tr>
  </tbody>
</table>
